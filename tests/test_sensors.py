...
@pytest.mark.parametrize("unit", ("kWh", "MWh"))
@pytest.mark.parametrize("trade", ("spot", "buy", "sell"))
async def test_interval_order_cross_midnight(
    hass: HomeAssistant,
    mock_ote_electricity: AsyncMock,
    mock_cnb: AsyncMock,
    currency: str,
    ...
    with freeze_time(BASE_DT):
        ...
        sensor = hass.states.get(f"sensor.current_{trade}_electricity_15min_order")
        assert sensor
        assert sensor.state is not None
        attr = cast(dict[str, Any], sensor.attributes)
        assert attr["2025-10-22T00:00:00+02:00"] is not None
